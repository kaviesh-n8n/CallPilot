...
The API Trigger node lets you initiate outbound calls to your voice agent programmatically. When you add an API Trigger node to your workflow, CallPilot generates a unique endpoint URL that external systems can call to start a conversation.
...
When you add an API Trigger node to your workflow, CallPilot assigns it a unique UUID. The trigger node exposes two URLs that share this UUID — one for the published agent and one for the latest draft. You can copy either URL from the trigger node's settings dialog.

```
POST https://your-callpilot-instance/api/v1/public/agent/{uuid}        # Production
POST https://your-callpilot-instance/api/v1/public/agent/test/{uuid}   # Test
```

If you are using the hosted version, replace `your-callpilot-instance` with `api.callpilot.ai`.
...
```bash
curl -X POST https://your-callpilot-instance/api/v1/public/agent/{uuid} \
...
The id is shown on each row in **Telephony configurations** (`https://app.callpilot.ai/telephony-configurations` for hosted or `http://localhost:3010/telephony-configurations` for local). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.
