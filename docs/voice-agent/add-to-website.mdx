...
description: Add your CallPilot voice agent to any website so visitors can talk to it.
tag: "NEW"
---
...
| **Inline Component**  | A panel rendered inside a `<div id="callpilot-inline-container">` that you place in your page.        | You want the agent embedded in a specific section (landing-page hero, support tab, etc.).            |
| **Headless**          | No UI. Only the audio pipeline plus a JavaScript API on `window.CallPilotWidget`.                     | You want full control over the UI — your own buttons, design system, framework state, animations.    |
...
- The embed snippet you copy from the dashboard is a single `<script>` tag that loads `callpilot-widget.js` **asynchronously**. The widget auto-initializes once it loads and exposes `window.CallPilotWidget`. Code that registers callbacks must wait for the widget to be available.
...
```html
<!-- Paste the callpilot embed snippet from the dashboard somewhere on the page -->
<div id="callpilot-inline-container"></div>
```

### React

Because React mounts after the widget script may have already loaded, integrate via `initInline` on first mount and `refresh` on remount. Poll for `window.CallPilotWidget` to handle the async script load.
...
export function Assistant() {
  useEffect(() => {
    let retries = 0;
    const tryInit = () => {
      const container = document.getElementById('callpilot-inline-container');
      if (window.CallPilotWidget && container) {
        const { isInitialized } = window.CallPilotWidget.getState();
        if (isInitialized) window.CallPilotWidget.refresh();
        else window.CallPilotWidget.initInline({ container });
      } else if (retries++ < 50) {
  ...
  return <div id="callpilot-inline-container" />;
}
```
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `window.CallPilotWidget.start()`                  | Begin a voice call. Must be called from inside a user-gesture handler (e.g. `click`) so the browser grants microphone access.                                |
| `window.CallPilotWidget.end()`                    | End the active call.                                                                                                                                         |
| `window.CallPilotWidget.onCallStart(cb)`          | Fires when `start()` is invoked (status `connecting`). No payload.                                                                                           |
| `window.CallPilotWidget.onCallConnected(cb)`      | Fires when the WebRTC connection is established. Payload: `{ agentId, workflowRunId, token }`.                                                               |
| `window.CallPilotWidget.onCallDisconnected(cb)`   | Fires only if the call had connected, when teardown runs. Payload: `{ agentId, workflowRunId, token, durationSeconds }`.                                     |
| `window.CallPilotWidget.onCallEnd(cb)`            | Fires whenever the call session is torn down (including failed-to-connect attempts). No payload.                                                             |
| `window.CallPilotWidget.onStatusChange(cb)`       | Fires on every status change. Callback receives `(status, text, subtext)`. Status values: `idle`, `connecting`, `connected`, `failed`.                       |
| `window.CallPilotWidget.onError(cb)`              | Fires on errors (mic permission denied, server error, etc.). Callback receives an `Error` object.                                                            |

All `on*` setters are single-listener — calling the same one again replaces the previous handler.

<Note>
**About timing.** The widget script loads asynchronously, so `window.CallPilotWidget` may not exist at the moment your inline `<script>` first runs. The examples below assume `window.CallPilotWidget` is already available when registration runs. To guarantee that:
...
<script>
  ...
  window.CallPilotWidget.onError((err) => {
    console.error('CallPilot error:', err.message);
  });
...
export function TalkButton() {
  ...
  useEffect(() => {
    window.CallPilotWidget.onStatusChange((s) => setStatus(s));
    window.CallPilotWidget.onError((err) => console.error('CallPilot error:', err.message));
  }, []);
...
```js
window.CallPilotWidget.onCallConnected(({ agentId, workflowRunId }) => {
  analytics.track('voice_call_started', { agentId, workflowRunId });
});

window.CallPilotWidget.onCallDisconnected(({ workflowRunId, durationSeconds }) => {
