---
title: "Update"
description: "Update your self-hosted CallPilot stack to a newer image version"
---

This guide covers updating a CallPilot stack you've already deployed with [Docker](/deployment/docker) or a [custom domain](/deployment/custom-domain). You run commands from the same directory that contains your `docker-compose.yaml` (this is the `callpilot/` directory if you used `setup_remote.sh`).
...
CallPilot publishes two images — `dograh-api` and `dograh-ui` — to both container registries:
...
|-------|-----------|---------|-------------|
| GitHub release tag | `dograh-vX.Y.Z` | `dograh-v1.28.0` | What you see at [github.com/kaviesh-n8n/CallPilot/releases](https://github.com/kaviesh-n8n/CallPilot/releases) |
...
```bash
cd callpilot
curl -o update_remote.sh https://raw.githubusercontent.com/kaviesh-n8n/CallPilot/main/scripts/update_remote.sh
...
<Warning>
Rolling back across a database migration is not always safe — if the newer release ran a schema migration, downgrading may leave the DB in a state the older API doesn't understand. If in doubt, [open an issue](https://github.com/kaviesh-n8n/CallPilot/issues) before rolling back.
</Warning>
