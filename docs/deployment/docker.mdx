---
title: "Docker"
description: "Deploy CallPilot AI using Docker for local development and remote server environments"
---

CallPilot AI can be deployed using Docker in two main configurations. Choose the option that best fits your needs:

- **Option 1**: For local development and testing on your own machine
- **Option 2**: For remote server deployment with HTTPS (using IP address). If you also have a custom domain, you can first deploy CallPilot stack on your server using steps in this document and then proceed to the [Custom Domain](deployment/custom-domain) section.

## Option 1: Local Docker Deployment

Watch the video tutorial below for a step-by-step walkthrough of setting up CallPilot AI locally with Docker.

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.tella.tv/video/dograh-local-setup-with-docker-5wsf/embed?b=1&title=1&a=1&loop=0&t=0&muted=0&wt=1"
  title="CallPilot Local Setup with Docker"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowFullScreen
></iframe>

For local development and testing, you can run CallPilot AI directly on your machine using Docker with a single command.

### Quick Start

Run this single command to download and start CallPilot AI:

```bash
curl -o docker-compose.yaml https://raw.githubusercontent.com/kaviesh-n8n/CallPilot/main/docker-compose.yaml && REGISTRY=ghcr.io/dograh-hq ENABLE_TELEMETRY=true docker compose up --pull always
```
...
<CodeGroup>
```bash macOS/Linux
curl -o setup_local.sh https://raw.githubusercontent.com/kaviesh-n8n/CallPilot/main/scripts/setup_local.sh && chmod +x setup_local.sh && ./setup_local.sh
```
```powershell Windows
Invoke-WebRequest -OutFile setup_local.ps1 https://raw.githubusercontent.com/kaviesh-n8n/CallPilot/main/scripts/setup_local.ps1
.\setup_local.ps1
```
</CodeGroup>
...
```bash
curl -o setup_remote.sh https://raw.githubusercontent.com/kaviesh-n8n/CallPilot/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && ./setup_remote.sh
```
...
After the setup script completes, start CallPilot. The script prints the exact command to run at the end — it differs slightly between modes:
...
The setup script creates the following files in the `callpilot/` directory:
...
1. Clones the repo into `callpilot/` (default: `kaviesh-n8n/CallPilot@main`; the script will ask for an `owner/name` and `branch` so a fork can point at its own).
