---
title: "Custom Domain"
description: "Deploy CallPilot AI with custom domain names and SSL certificates"
---

Deploy CallPilot AI with your own custom domain name for a professional production setup. By now, you should be able to create and test a voice agent by following the previous guide to setup the platform on a remote server using [Docker](docker#option-2%3A-remote-server-deployment)

## What is Custom Domain Deployment?

Custom domain deployment allows you to run CallPilot AI with a personalized domain name (like `voice.yourcompany.com`) instead of using IP addresses. This setup includes:
...
- CallPilot AI already running on your server via the [remote deployment](docker#option-2%3A-remote-server-deployment) guide
...
<Note>
You must be at the same place where you had run `setup_remote.sh` from. The directory should contain `callpilot/` with the artifacts that got created when `setup_remote.sh` was run.
</Note>
<Note>
You must not move the `callpilot/` directory to a different location after running `setup_custom_domain.sh`, since we set up auto certificate renewal script as certbot renewal hook pointing to the `callpilot/` directory.
</Note>

```bash
curl -o setup_custom_domain.sh https://raw.githubusercontent.com/kaviesh-n8n/CallPilot/main/scripts/setup_custom_domain.sh && chmod +x setup_custom_domain.sh && sudo ./setup_custom_domain.sh
```
...
- Restart CallPilot services through the validated startup wrapper
...
### Stop CallPilot Services

Before generating certificates, stop the running CallPilot services to free up port 80:
...
### Copy Certificates to CallPilot Directory
...
### Start CallPilot Services

Start CallPilot through the validated startup wrapper so `dograh-init` regenerates nginx and coturn runtime config before Docker starts:
...
cp /etc/letsencrypt/live/voice.yourcompany.com/fullchain.pem /home/ubuntu/callpilot/certs/local.crt
cp /etc/letsencrypt/live/voice.yourcompany.com/privkey.pem /home/ubuntu/callpilot/certs/local.key
chmod 644 /home/ubuntu/callpilot/certs/local.crt /home/ubuntu/callpilot/certs/local.key
...
1. Verify the certificates were copied correctly: `ls -la callpilot/certs/`
2. Run `./remote_up.sh --preflight-only` in `callpilot/` to verify the `dograh-init` runtime render matches `.env`
