...
---
<Note>
If the below steps do not work out for you, it would be great if you can open an issue on [Github](https://github.com/kaviesh-n8n/CallPilot/issues).
</Note>
...
### Steps
1. Fork the CallPilot repository by going to https://github.com/kaviesh-n8n/CallPilot
...
cd callpilot
```
3. Run the contributor bootstrap. It configures `origin` (your fork) and `upstream` (`kaviesh-n8n/CallPilot`), initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates. Re-running it is safe — already-configured pieces are skipped.
<CodeGroup>
```bash macOS/Linux
...
The bootstrap script configures two remotes: `origin` (your fork, where you push) and `upstream` (`kaviesh-n8n/CallPilot`, where new commits land). To pull in upstream changes:
...
upstream  https://github.com/kaviesh-n8n/CallPilot.git    (fetch/push)
```

<Note>
Always push feature branches to **`origin`** (your fork), then open a pull request against `kaviesh-n8n/CallPilot:main`. Never push directly to `upstream`.
</Note>
