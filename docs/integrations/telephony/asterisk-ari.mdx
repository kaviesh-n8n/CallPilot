...
description: "Connect CallPilot AI to your Asterisk PBX using the Asterisk REST Interface (ARI)"
---

## Overview

Asterisk ARI (Asterisk REST Interface) allows you to connect CallPilot AI voice agents to your existing Asterisk PBX. ARI provides a WebSocket-based event model for controlling calls via Stasis applications, giving CallPilot full control over call flow and audio streaming.

This guide focuses on the CallPilot-specific configuration. For general Asterisk installation and administration, refer to the [official Asterisk documentation](https://docs.asterisk.org/).
...
- Network connectivity between your CallPilot instance and Asterisk
- CallPilot AI instance running and accessible

<Note>
If you compiled Asterisk from source, ensure `chan_websocket` is included during the build. This module is required for external media streaming between Asterisk and CallPilot. Refer to the [Asterisk build system documentation](https://docs.asterisk.org/) for details on enabling modules.
</Note>

## Asterisk Configuration

The following Asterisk configuration files need to be set up to work with CallPilot. These are minimal examples focused on the CallPilot integration -- refer to the [Asterisk documentation](https://docs.asterisk.org/) for full configuration details.

### Enable ARI (`ari.conf`)

Create an ARI user that CallPilot will use to authenticate:
...
<Note>
The username (section name, e.g., `dograh`) and password here must match the **Stasis App Name** and **App Password** you configure in CallPilot.
</Note>
...
Route incoming calls to your Stasis application so CallPilot can handle them:
...
Replace `dograh` with the app name you configured in `ari.conf` and in CallPilot.

### Configure External Media Streaming (`websocket_client.conf`)

CallPilot uses Asterisk's external media streaming to send and receive audio over WebSocket. Configure a WebSocket client connection that points to your CallPilot instance:

```ini
[dograh]
type = websocket_client
uri = ws://your-callpilot-host:port/api/v1/telephony/ws/ari
protocols = audio
```

<Note>
The section name (e.g., `dograh`) is the **WebSocket Client Name** you'll enter in the CallPilot telephony configuration. This name tells Asterisk which WebSocket connection to use for external media streaming during calls.
</Note>
...
Unlike other telephony providers that use HTTP webhooks for inbound calls, ARI delivers inbound calls as **StasisStart events on the ARI WebSocket**. CallPilot automatically detects these events and activates the workflow assigned to the called extension.
...
3. CallPilot looks up the called extension in your telephony configuration's phone numbers, finds the assigned workflow, validates quota, and creates a workflow run
...
…or use a pattern that catches every extension you'll register in CallPilot:
...
**Step 2: Add the extension as a phone number in CallPilot**
...
4. Save
   ...
   <Note>
   Adding the extension in CallPilot doesn't change Asterisk's dialplan — that's
   what Step 1 is for. The CallPilot entry tells the StasisStart handler which
...
<AccordionGroup>
  <Accordion title="Cannot connect to ARI endpoint">
    - Verify the ARI endpoint URL is correct and reachable from your CallPilot instance
  ...
  <Accordion title="No audio during calls">
    ...
    - Check that `websocket_client.conf` is correctly configured with the right CallPilot URI
    - Ensure the WebSocket Client Name in CallPilot matches the section name in `websocket_client.conf`
    - Verify network connectivity and firewall rules allow WebSocket traffic between Asterisk and CallPilot
  </Accordion>

  <Accordion title="Calls not reaching CallPilot">
  ...
  <Accordion title="Inbound calls are immediately hung up">
    ...
    - Review CallPilot logs for warnings like "no matching phone number registered
    for config" or "has no inbound_workflow_id assigned"
  </Accordion>

  <Accordion title="WebSocket client connection issues">
    - Check the URI in `websocket_client.conf` points to the correct CallPilot host and port
    - Verify the CallPilot instance is running and accepting WebSocket connections
...
- Keep your Asterisk instance on the same network or a low-latency connection to CallPilot for optimal audio quality
- Use strong passwords for ARI authentication
- Restrict ARI access to known IP addresses using firewall rules
- Monitor Asterisk logs alongside CallPilot logs when debugging call issues
