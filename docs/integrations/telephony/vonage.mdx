...
description: "Configure Vonage (Nexmo) for voice communication in CallPilot AI"
---

## Overview

Vonage (formerly Nexmo) is a cloud communications platform that provides global voice, messaging, and video capabilities. CallPilot AI's Vonage integration enables high-quality voice interactions with your agents using Vonage's robust infrastructure.
...
### Step 3: Configure in CallPilot AI
...
Vonage configures inbound webhooks at the **application level**, not per phone number. A single **Answer URL** on the Vonage application applies to every number linked to it. CallPilot routes the call to the right agent based on the called number's inbound workflow assignment inside CallPilot. **When you save an inbound workflow on a phone number, CallPilot automatically pushes the webhook URL to your Vonage Application's Answer URL** (provided the credentials are correct).

### Step 1: Link Phone Numbers to Your Vonage Application

1. Open the [Vonage Dashboard](https://dashboard.nexmo.com/)
2. Under **Numbers** → **Your Numbers**, link each number you want to use for inbound to the same Vonage Application whose ID you configured in CallPilot
...
2. Under **Capabilities** → **Voice**, confirm:
   ...
   <Note>
     ...
     in Step 2. If the field is empty, shows a different URL, or CallPilot
     surfaced a sync warning on save, the auto-push failed — most often
     because the API Key/Secret or Application ID in CallPilot is incorrect.
     Paste the URL into the field yourself, set the method to `POST`, and
     save the application. On self-hosted CallPilot, replace `api.callpilot.ai`
...
3. Check call logs in both CallPilot AI dashboard and Vonage Dashboard
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls not reaching voice agent">
    - Verify the Vonage application's Answer URL is set to `https://api.callpilot.ai/api/v1/telephony/inbound/run`
    - Ensure the Answer URL is publicly accessible
    - Confirm the called number is linked to the correct Vonage application
    - Confirm the called number exists in your CallPilot telephony configuration and has an **Inbound workflow** assigned
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    - Verify API Key matches the one stored in your CallPilot telephony configuration (used to identify the org from the inbound webhook)
