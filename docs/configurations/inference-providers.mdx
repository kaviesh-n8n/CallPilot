...
description: "Voice Agents need AI Models to work, like LLM (Large Language Model), TTS (Voice) and STT (Transcriber). You can use any of your faviourite providers with CallPilot Platform to run your Voice Agent."
---

## How Model Configuration Works

CallPilot uses a **two-level configuration system** for AI models:
...
The global configuration is the default model setup shared across all your agents. CallPilot ships with its own models by default — when you sign up on https://app.callpilot.ai or set up the platform on your self-hosted infrastructure, you get some CallPilot model credits to start with.

To configure the global models, go to **Model Configurations** in your dashboard:
- **Hosted:** `https://app.callpilot.ai/model-configurations`
...
Select a provider from the dropdown and configure the API key, model, and any provider-specific settings. For CallPilot's own models, see [Service Keys](api-keys) for instructions on creating Service Keys.
...
CallPilot supports Gemini 3.1 Live as a **Realtime** provider. The default model is `gemini-3.1-flash-live-preview`.
...
To use Gemini 3.1 Live with CallPilot, you need a Google Gemini API key. Follow these steps:
...
<Note>
  The Gemini API key is different from a Google Cloud service account key. You specifically need a **Gemini API key** from Google AI Studio for use with CallPilot.
</Note>

### Configuring Gemini 3.1 Live in CallPilot

1. Go to **Model Configurations** in your CallPilot dashboard (`https://app.callpilot.ai/model-configurations` for hosted or `http://localhost:3010/model-configurations` for local).
...
<Note>
  Always pick **JSON**, not P12. The Vertex AI client libraries used by CallPilot only accept service-account JSON keys; P12 is a legacy format retained for older Google Workspace integrations.
</Note>

### Configuring Vertex AI Realtime in CallPilot

1. Go to **Model Configurations** in your CallPilot dashboard.
...
<Note>
  Paste the whole JSON file into the **Credentials** field — including `private_key`, `client_email`, and all other entries. Don't try to extract individual fields. If `Credentials` is left blank, CallPilot falls back to **Application Default Credentials (ADC)** from the host environment, which is useful when running CallPilot on a GCP VM or GKE pod with an attached service account.
</Note>
