...
description: "Trigger a CallPilot voice agent to call a phone number from the SDK"
---

Use the SDK to place a test outbound call from a specific agent to a phone number. This is the same endpoint used by the **Test Call** button in the CallPilot UI.

## Prerequisites

- A CallPilot [API key](/configurations/api-keys) exported as `CALLPILOT_API_KEY`
...
from callpilot_sdk._generated_models import InitiateCallRequest

with CallPilotClient(api_key="YOUR_API_KEY") as client:
...
const client = new CallPilotClient({ apiKey: "YOUR_API_KEY" });
