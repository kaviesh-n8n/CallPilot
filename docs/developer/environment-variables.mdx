...
description: "Complete reference for all environment variables used by the CallPilot backend"
---
...
CallPilot supports two deployment modes, set via `DEPLOYMENT_MODE`:

- **OSS**: The default mode. Designed for self-hosted deployments using [Docker Compose](/deployment/docker) — the fastest way to get CallPilot running. Uses local JWT authentication and MinIO for storage.
- **SaaS**: Intended for customised deployments outside of Docker. Authentication and API key management are handled through CallPilot Managed Platform Services (MPS), allowing greater flexibility in how the platform is hosted and integrated.
...
| Variable | Default | Description |
|---|---|---|
| `BACKEND_API_ENDPOINT` | `RENDER_EXTERNAL_URL` or `http://localhost:8000` | Public backend URL used for webhooks, downloads, and WebRTC callbacks. On Render, CallPilot automatically falls back to `RENDER_EXTERNAL_URL` when `BACKEND_API_ENDPOINT` is unset. |
| `UI_APP_URL` | `http://localhost:3010` | URL of the frontend application |
| `MPS_API_URL` | `https://services.dograh.com` | CallPilot Managed Platform Services URL |
...
CallPilot uses **MinIO by default**, which is bundled with the self-hosted deployment and requires no external setup. Set `ENABLE_AWS_S3=true` to switch to AWS S3 — typically used for cloud or managed deployments where S3 is already part of the infrastructure.
...
| `S3_ENDPOINT_URL` | `null` | Optional S3-compatible endpoint URL. Set this for Cloudflare R2 or other non-AWS S3 providers. |
| `S3_BUCKET` | `null` | S3 bucket name |
| `S3_REGION` | `us-east-1` | AWS region, or `auto` when using Cloudflare R2 |

Cloudflare R2 works by setting `ENABLE_AWS_S3=true`, `S3_ENDPOINT_URL=https://<ACCOUNT_ID>.r2.cloudflarestorage.com`, and `S3_REGION=auto`.
