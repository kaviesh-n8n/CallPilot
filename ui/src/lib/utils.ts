...
export function setStackRefreshCookie(refreshToken: string) {
  ...
  const isCallPilotDomain = window.location.hostname.endsWith('.callpilot.ai');
  const cookieDomainPart = isCallPilotDomain ? '; domain=.callpilot.ai' : '';
...
export async function impersonateAsSuperadmin(params: {
  ...
  // (served from the target sub-domain, e.g. app.callpilot.ai). The route will set the
  ...
  // superadmin.callpilot.ai we want to switch to app.callpilot.ai. For any other domain
